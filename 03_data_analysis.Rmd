...
```{r beta_div_nmds_richness_plot, comment="", message=FALSE, warning=FALSE}
richness %>%
...
Model_neutral <- lme(fixed = neutral ~ treatment, data = alpha_div_meta,
               random = ~ 1 | individual)
summary(Model_neutral)
emmeans::emmeans(Model_neutral, pairwise ~ treatment)
```
...
Model_phylo <- lme(fixed = phylogenetic ~ treatment, data = alpha_div_meta,
               random = ~ 1 | individual)
summary(Model_phylo)
emmeans::emmeans(Model_phylo, pairwise ~ treatment)
```
