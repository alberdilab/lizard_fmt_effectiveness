...
```{r permanova_richness_accli_1, comment="", message=FALSE, warning=FALSE}
...
```{r permanova_neutral_accli_1, comment="", message=FALSE, warning=FALSE}
subset_meta_arrange <- column_to_rownames(subset_meta, "Tube_code")
...
```{r permanova_phylo_accli_1, comment="", message=FALSE, warning=FALSE}
subset_meta_arrange <- column_to_rownames(subset_meta, "Tube_code")
...
### Differential abundance analysis: composition
```{r phylo_acclim_wild_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancom_rand1_wild_accli_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ancombc_rand_plot_phy1_wild_accli_warm, comment="", message=FALSE, warning=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE, eval=FALSE}
ancombc_rand_table_mag%>%
      mutate(genome=factor(taxon,levels=ancombc_rand_table_mag$taxon)) %>%
ggplot(., aes(x=lfc_time_point1_Acclimation, y=forcats::fct_reorder(genome,lfc_time_point1_Acclimation), fill=phylum)) + #forcats::fct_rev()
  geom_col() + 
...
***Phyla of the significant MAGs***
```{r acclim_sign_features_wild_accli_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_time_point1_Acclimation<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
