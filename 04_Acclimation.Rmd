...
```{r beta_neutral_nmds_plot_final, fig.height=8, fig.width=12, fig.fullwidth=TRUE}
...
ancombc_wild_accl_1203 = ancombc2(data = physeq_genome_filtered, 
                  assay_name = "counts", 
                  tax_level = NULL,
...
save(ancombc_wild_accl_1203, 
     file = "data/ancombc_wild_accl_1203.Rdata")
```

```{r load_ancombc_wild_accli, eval=TRUE, warning=FALSE, comments="", message=FALSE}
load("data/ancombc_wild_accl_1203.Rdata")
```
...
ancombc_rand_table_mag <- ancombc_wild_accl_1203$res %>%
  dplyr::select(taxon, lfc_treatmentWild, p_treatmentWild) %>%
  filter(p_treatmentWild < 0.05) %>%
  dplyr::arrange(p_treatmentWild) %>%
  merge(., taxonomy, by="taxon") %>%
  mutate_at(vars(phylum, species), ~ str_replace(., "[dpcofgs]__", ""))%>%
  dplyr::arrange(lfc_treatmentWild)
...
ggplot(., aes(x=lfc_treatmentWild, y=forcats::fct_reorder(genome,lfc_treatmentWild), fill=phylum)) + #forcats::fct_rev()
  geom_col() + 
...
```
```{r ancom_rand_accl_wild_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_wild_accl_warm_1203 = ancombc2(data = physeq_genome_filtered_warm, 
                  assay_name = "counts", 
                  tax_level = NULL,
                  fix_formula = "treatment",
                  rand_formula = "(1|individual)",
...
```{r save_ancombc1_wild_accli_warm, warning=FALSE, comments="", message=FALSE, eval=FALSE}
save(ancombc_wild_accl_warm_1203, 
     file = "data/ancombc_wild_accl_warm_1203.Rdata")
```

```{r load_ancombc_wild_accli_warm, warning=FALSE, comments="", message=FALSE}
load("data/ancombc_wild_accl_warm_1203.Rdata")
```
...
ancombc_rand_table_mag <- ancombc_wild_accl_warm_1203$res %>%
  dplyr::select(taxon, lfc_treatmentWild, p_treatmentWild) %>%
  filter(p_treatmentWild < 0.05) %>%
  dplyr::arrange(p_treatmentWild) %>%
  merge(., taxonomy, by="taxon") %>%
...
***Phyla of the significant MAGs***
```{r acclim_sign_features_wild_accli_warm, comment="", message=FALSE, warning=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_treatmentWild<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
