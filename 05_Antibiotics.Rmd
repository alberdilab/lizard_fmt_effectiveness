...
## Differences in MAGs and functional capacity
...
Acclimation
```{r acclim_sign_features_acl, comment="", message=FALSE, warning=FALSE}
ancombc_rand_table_mag%>%
...
Antibiotics
```{r acclim_sign_features_ant, comment="", message=FALSE, warning=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_time_pointAntibiotics>0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
difference_table <- function_gift_filt %>%
  ...
  mutate(Difference=Acclimation-Antibiotics)%>% 
  mutate(time_point = ifelse(Difference <0, "Antibiotics","Acclimation")) 
...
***Phyla of the significant MAGs***
Acclimation
```{r acclim_sign_features_warm2, comment="", message=FALSE, warning=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_time_pointAntibiotics<0)  %>% 
  select(phylum, genus, species)
```
...
Antibiotics
```{r antib_sign_features_warm2, comment="", message=FALSE, warning=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_time_pointAntibiotics>0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
