...
```{r permanova_richness_antib, comment="", message=FALSE, warning=FALSE}
...
```{r permanova_neutral_antib, comment="", message=FALSE, warning=FALSE}
subset_meta_arrange <- column_to_rownames(subset_meta, "Tube_code")
...
```{r permanova_phylo_antib, comment="", message=FALSE, warning=FALSE}
subset_meta_arrange <- column_to_rownames(subset_meta, "Tube_code")
...
```{r beta_neutral_rda_plot_final, fig.height=8, fig.width=12, fig.fullwidth=TRUE}
...
```
```{r ancom_rand_accl_ant, comment="", message=FALSE, warning=FALSE}
...
```{r save_ancombc1, warning=FALSE, comments="", message=FALSE}
save(ancom_rand_output, 
     file = "data/ancombc_antib_accl_1203.Rdata")
```

```{r load_ancombc, warning=FALSE, comments="", message=FALSE, eval=FALSE}
load("data/ancombc_antib_accl_1203.Rdata")
```
...
#### Structural zeros
```{r struct_zero_warm, comment="", message=FALSE, warning=FALSE}
acclimation_samples <- sample_metadata %>%
...
***Acclimation***
```{r structural_zeros_accli_warm, comment="", message=FALSE, warning=FALSE}
structural_zeros %>% 
  filter(present=="acclimation") %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count)) 
...
***Antibiotics***
```{r structural_zeros_antib_warm, comment="", message=FALSE, warning=FALSE}
structural_zeros %>% 
  filter(present=="antibiotics") %>% 
  select(1,5:10)
```
...
***Community elements differences in structural zeros***
```{r gift_struc_acclim_antib_warm, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_elements <- to.elements(genome_gifts,GIFT_db)
GIFTs_elements_filtered <- GIFTs_elements[rownames(GIFTs_elements) %in% structural_zeros$genome,]
GIFTs_elements_filtered <- as.data.frame(GIFTs_elements_filtered) %>% 
  select_if(~ !is.numeric(.) || sum(.) != 0)
...
```{r comunity_elem_acclim_antib_struc_warm, comment="", message=FALSE, warning=FALSE}
element_gift <- GIFTs_elements_community %>% 
  as.data.frame() %>% 
  rownames_to_column(., "sample") %>% 
  left_join(., sample_metadata[c(12,13)], by=join_by("sample"=="sample"))
...
```{r commun_wilcox_struc_acclim_antib_warm, comment="", message=FALSE, warning=FALSE}
uniqueGIFT_db<- unique(GIFT_db[c(2,4,5,6)]) %>% unite("Function",Function:Element, sep= "_", remove=FALSE)
...
#### Differential abundance analysis: composition
```{r phylo_acclim_antib_warm, comment="", message=FALSE, warning=FALSE}
phylo_samples <- sample_metadata %>%
...
```{r ancom_rand_accl_ant_warm, comment="", message=FALSE, warning=FALSE}
ancom_rand_output = ancombc2(data = physeq_genome_filtered, 
                  assay_name = "counts", 
                  tax_level = NULL,
                  fix_formula = "time_point",
...
```{r save_ancombc1_warm, warning=FALSE, comments="", message=FALSE, eval=FALSE}
save(ancom_rand_output, 
     file = "data/ancombc_antib_accl_warm_1203.Rdata")
```

```{r load_ancombc_warm, warning=FALSE, comments="", message=FALSE, eval=FALSE}
load("data/ancombc_antib_accl_warm_1203.Rdata")
```

```{r ancom_rand1_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
taxonomy <- data.frame(physeq_genome_filtered@tax_table) %>%
  rownames_to_column(., "taxon") %>%
  mutate_at(vars(order, phylum, family, genus, species), ~ str_replace(., "[dpcofgs]__", ""))
...
tax_color <- merge(tax_table, colors_alphabetic, by="phylum")%>%
	...
	pull()
```
```{r ancombc_rand_plot_phy1_warm, comment="", message=FALSE, warning=FALSE, fig.height=10, fig.width=8, fig.fullwidth=TRUE, eval=FALSE}
ancombc_rand_table_mag%>%
      mutate(genome=factor(taxon,levels=ancombc_rand_table_mag$taxon)) %>%
...
***Phyla of the significant MAGs***
```{r acclim_sign_features_warm, comment="", message=FALSE, warning=FALSE, eval=FALSE}
ancombc_rand_table_mag%>%
  filter(lfc_time_pointAntibiotics<0)  %>% 
  count(phylum, name = "sample_count") %>%
  arrange(desc(sample_count))  
...
#### Differences in the functional capacity
```{r gift_acclim_antib_warm, comment="", message=FALSE, warning=FALSE}
GIFTs_elements <- to.elements(genome_gifts,GIFT_db)
GIFTs_elements_filtered <- GIFTs_elements[rownames(GIFTs_elements) %in% genome_counts_filt$genome,]
GIFTs_elements_filtered <- as.data.frame(GIFTs_elements_filtered) %>% 
  select_if(~ !is.numeric(.) || sum(.) != 0)
...
***MCI***
```{r gitfs_elem_accli_antib_warm, comment="", message=FALSE, warning=FALSE}
GIFTs_elements_community %>%
  rowMeans() %>%
  as_tibble(., rownames = "sample") %>%
  left_join(sample_metadata, by = join_by(sample == sample)) %>%
...
```{r comunity_elem_acclim_antib_warm, comment="", message=FALSE, warning=FALSE}
element_gift <- GIFTs_elements_community %>% 
  as.data.frame() %>% 
  rownames_to_column(., "sample") %>% 
  left_join(., sample_metadata[c(12,13)], by=join_by("sample"=="sample"))
...
```{r commun_wilcox_elem_acclim_antib_warm, comment="", message=FALSE, warning=FALSE}
uniqueGIFT_db<- unique(GIFT_db[c(2,4,5,6)]) %>% unite("Function",Function:Element, sep= "_", remove=FALSE)
...
```{r commun_wilcox_elem_plot_acclim_antib_warm, comment="", message=FALSE, warning=FALSE, fig.height=12, fig.width=12, fig.fullwidth=TRUE}
difference_table %>%
  ggplot(aes(x=forcats::fct_reorder(Function,Difference), y=Difference, fill=group_color)) + 
  geom_col() +
#  geom_point(size=4) + 
...
***MCI***
```{r gitfs_accli_antib_warm, message=FALSE, warning=FALSE}
GIFTs_functions_community %>%
  rowMeans() %>%
  as_tibble(., rownames = "sample") %>%
  left_join(sample_metadata, by = join_by(sample == sample)) %>%
...
```{r comunity_func_Acci_anti_warm, comment="", message=FALSE, warning=FALSE}
function_gift <- GIFTs_functions_community %>% 
  as.data.frame() %>% 
  rownames_to_column(., "sample") %>% 
  merge(., sample_metadata[c(12,13)], by="sample")
...
```{r commun_wilcox_func_warm, comment="", message=FALSE, warning=FALSE}
unique_funct_db<- GIFT_db[c(3,4,5)] %>% 
  distinct(Code_function, .keep_all = TRUE)
...
significant_functional <- function_gift %>%
  ...
  left_join(.,unique_funct_db[c(1,3)],by = join_by(trait == Code_function))
```
```{r  function_sig_warm, comment="", echo=FALSE, message=FALSE, warning=FALSE}
function_gift_t <- function_gift  %>% 
  select(-treatment)  %>% 
  t() %>%
  row_to_names(row_number = 1) %>%
