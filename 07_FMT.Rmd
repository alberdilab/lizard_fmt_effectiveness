...
alpha_div_meta <- alpha_div %>%
  left_join(sample_metadata, by = join_by(sample == sample))%>%
  filter(type == "Treatment" & treatment %in% c("Antibiotics", "Acclimation","FMT1","FMT2") ) 
```
...
alpha_div %>%
  ...
  scale_color_manual(name="Treatment",
          breaks=c("Antibiotics","Acclimation", "FMT1","FMT2"),
          values=c("#003a45",'#008080', "#76b183",'#40714b50')) +
      scale_fill_manual(name="Treatment",
          breaks=c("Antibiotics","Acclimation", "FMT1","FMT2"),
          values=c("#003a4550",'#00808050',"#76b18350",'#40714b50')) +
  facet_wrap(. ~ metric, scales = "free") +
      theme_classic() +
        theme(
    strip.background = element_blank(),
...
samples_to_keep <- sample_metadata %>%
  filter(type == "Treatment" & treatment %in% c("Acclimation", "Antibiotics", "FMT1", "FMT2")) %>% 
  select(sample) %>% 
  pull()
subset_meta <- sample_metadata %>%
  filter(type == "Treatment" & treatment %in% c("Acclimation", "Antibiotics", "FMT1", "FMT2"))
...
richness %>%
  ...
  geom_point(size = 4) +
  scale_color_manual(name="treatment",
          breaks=c("Acclimation", "Antibiotics", "FMT1", "FMT2"),
          values=c("#003a45",'#008080', "#76b183",'#40714b50')) +
  geom_segment(aes(x = x_cen, y = y_cen, xend = NMDS1, yend = NMDS2), alpha = 0.9, show.legend = FALSE) +
  theme(
    axis.title = element_text(size = 12, face = "bold"),
...
alpha_div_meta <- alpha_div %>%
  left_join(sample_metadata, by = join_by(sample == sample))%>%
  filter(type == "Treatment" & treatment %in% c("Transplant", "FMT1", "FMT2") )

alpha_div_meta$treatment <- factor(alpha_div_meta$treatment, levels=c("Transplant", "FMT1", "FMT2"))
```
...
alpha_div %>%
  ...
  geom_boxplot(width = 0.5, alpha=0.5, outlier.shape = NA, show.legend = FALSE) +
  scale_color_manual(name="treatment",
          breaks=c("FMT1","FMT2","Transplant"),
          values=c('#40714b', "#76b183", "#d5b52c")) +
  scale_fill_manual(name="treatment",
          breaks=c("FMT1","FMT2","Transplant"),
          values=c('#40714b50', "#76b183", "#d5b52c50")) +
  facet_wrap(. ~ metric, scales = "free") +
      theme_classic() +
        theme(
    strip.background = element_blank(),
...
sample_sub <- sample_metadata %>%
  filter(type == "Treatment" & treatment %in% c("FMT1","FMT2", "Transplant"))
...
FMT_samples <- sample_metadata %>%
  filter(type == "Treatment" & treatment == "FMT1") %>% 
  dplyr::select(sample) %>%
  pull()
...
sample_sub <- sample_metadata %>%
  filter(type == "Treatment" & treatment %in% c("FMT1", "Transplant"))
...
structural_zeros <- genome_counts_filt %>% 
  ...
  select(genome, where(~ is.numeric(.) && sum(.) > 0)) %>% 
   ...
   mutate(present = case_when(
      ...
      !all_zeros_FMT & all_zeros_Transplant ~ "FMT1",
      !all_zeros_FMT & !all_zeros_Transplant ~ "None",
      TRUE ~ NA_character_
    )) %>%
   mutate(average = ifelse(present == "FMT1", average_FMT, average_Transplant)) %>%
...
difference_table <- element_gift_filt %>%
  ...
  arrange(Function) %>% 
  mutate(Difference=FMT1-Transplant)%>% 
  mutate(group_color = ifelse(Difference <0, "Transplant","FMT1")) 
```
...
ancombc_FMT_transplant_1203 = ancombc2(data = physeq_genome_filtered, 
                  assay_name = "counts", 
                  tax_level = NULL,
...
save(ancombc_FMT_transplant_1203, file = "data/ancombc_FMT_transplant_2003.Rdata")
```

```{r load_ancombc_fmt_tranp, warning=FALSE, comments="", message=FALSE, eval=FALSE}
load("data/ancombc_FMT_transplant_2003.Rdata")
```
...
ancombc_rand_table_mag <- ancombc_FMT_transplant_1203$res %>%
...
FMT2_samples <- sample_metadata %>%
  filter(type == "Treatment" & treatment == "FMT2") %>% 
  dplyr::select(sample) %>%
  pull()
...
sample_sub <- sample_metadata %>%
  filter(type == "Treatment" & treatment %in% c("FMT2", "Transplant"))
...
structural_zeros <- genome_counts_filt %>% 
  ...
  select(genome, where(~ is.numeric(.) && sum(.) > 0)) %>% 
   ...
   mutate(average_Transplant = mean(c_across(all_of(Transplant_samples)), na.rm = TRUE)) %>% 
   filter(all_zeros_FMT2 == TRUE || all_zeros_Transplant==TRUE)  %>% 
   mutate(present = case_when(
      all_zeros_FMT2 & !all_zeros_Transplant ~ "Transplant",
      !all_zeros_FMT2 & all_zeros_Transplant ~ "FMT2",
      !all_zeros_FMT2 & !all_zeros_Transplant ~ "None",
      TRUE ~ NA_character_
    )) %>%
   mutate(average = ifelse(present == "FMT2", average_FMT2, average_Transplant)) %>%
   dplyr::select(genome, present, average) %>%
   left_join(genome_metadata, by=join_by(genome==genome)) %>%
   arrange(present,-average)
```
...
difference_table <- element_gift_filt %>%
  ...
  left_join(.,uniqueGIFT_db[c(1,3,4)],by = join_by(Elements == Code_element)) %>% 
  arrange(Function) %>% 
  mutate(Difference=FMT2-Transplant)%>% 
  mutate(group_color = ifelse(Difference <0, "Transplant","FMT2")) 
```
...
ancombc_FMT2_transplant_1203 = ancombc2(data = physeq_genome_filtered, 
                  assay_name = "counts", 
                  tax_level = NULL,
                  fix_formula = "treatment",
                  p_adj_method = "holm", 
...
save(ancombc_FMT2_transplant_1203, file = "data/ancombc_FMT2_transplant_2003.Rdata")
```

```{r load_ancombc_fmt2_tranp, warning=FALSE, comments="", message=FALSE, eval=FALSE}
load("data/ancombc_FMT2_transplant_2003.Rdata")
```
...
ancombc_rand_table_mag <- ancombc_FMT2_transplant_1203$res %>%
  dplyr::select(taxon, lfc_treatmentTransplant, p_treatmentTransplant) %>%
  filter(p_treatmentTransplant < 0.05) %>%
  dplyr::arrange(p_treatmentTransplant) %>%
  merge(., taxonomy, by="taxon") %>%
...
ancombc_rand_table_mag%>%
      mutate(genome=factor(taxon,levels=ancombc_rand_table_mag$taxon)) %>%
ggplot(., aes(x=lfc_treatmentTransplant, y=forcats::fct_reorder(genome,lfc_treatmentTransplant), fill=phylum)) +
  geom_col() + 
  scale_fill_manual(values=tax_color) + 
  geom_hline(yintercept=0) + 
  theme(panel.background = element_blank(),
